...
prep <- function(y.series, forecast.horizon, first.period, last.period, s.up, s.same, s.down) {
	...
	assign("survey.up", value=z, pos=1, envir=parent.frame())
	z <- s.same
	is.na(z[1:(first.period - 1)]) <- TRUE
	is.na(z[(last.period + 1):length(z)]) <- TRUE
	assign("survey.same", value=z, pos=1, envir=parent.frame())
	z <- s.down
	is.na(z[1:(first.period - 1)]) <- TRUE
	is.na(z[(last.period + 1):length(z)]) <- TRUE
	assign("survey.down", value=z, pos=1, envir=parent.frame())
	
	assign("y.e.mean.abs", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
	assign("y.e.mean.perc", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
	assign("delta.y.e.mean.abs", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
	assign("delta.y.e.mean.perc", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
	assign("delta.y.e.sd.abs", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
	assign("delta.y.e.sd.perc", c(rep(NA, length(s.up))), pos=1, envir=parent.frame())
}
...
ce<-function(y.series, survey.up, survey.same, survey.down, forecast.horizon, first.period = 11, last.period = (length(survey.up) - forecast.horizon), exp.horizon.type = "moving", mov.horizon.length = 10, fix.horizon.start = 1, fix.horizon.end = 10, distrib.param = "mean", suppress.warnings=FALSE) {
	...
	if (distrib.param != "mean" && distrib.param != "median") stop(gettextf("\n%s is no valid value for distr.param. distr.param must either be \"mean\" or \"median\".\n", distr.param))
